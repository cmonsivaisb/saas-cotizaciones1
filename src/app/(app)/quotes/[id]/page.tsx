                    ...
                    <div key={item.id} className="flex justify-between items-start p-4 bg-muted/50 rounded-lg">
                      <div className="flex-1">
                        <p className="font-medium">
                          {item.item?.name || item.description || 'Producto sin nombre'}
                          {item.item?.sku && <span className="text-sm text-muted-foreground ml-2">• SKU: {item.item.sku}</span>}
                        </p>
                        <p className="text-sm text-muted-foreground">
                          Cantidad: {item.qty} × ${item.unitPrice.toLocaleString('es-MX')}
                          {item.itemId && <span className="ml-2 text-xs bg-green-100 px-1 rounded">Del inventario</span>}
                        </p>
                      </div>
